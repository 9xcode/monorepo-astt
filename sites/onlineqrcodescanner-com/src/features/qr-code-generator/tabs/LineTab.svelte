<script lang="ts">
  ...
  const computed = $derived.by(() => {
    return buildSocialUrl(username, 'https://line.me/ti/p/~');
  });
...
<div class="space-y-5">
  <div class="space-y-1.5">
    <label for="line-input" class="block text-xs font-semibold uppercase tracking-wide text-muted-foreground">
      LINE ID <span class="text-emerald-500 normal-case tracking-normal font-medium ml-0.5">*</span>
    </label>
    <input
      id="line-input"
      type="text"
      bind:value={username}
      placeholder="LINE ID or full URL"
      class={I}
      spellcheck={false}
    />

<script lang="ts">
  ...
  const computed = $derived.by(() => {
    return buildSocialUrl(username, 'https://github.com/');
  });
...
<div class="space-y-5">
  <div class="space-y-1.5">
    <label for="github-input" class="block text-xs font-semibold uppercase tracking-wide text-muted-foreground">
      GitHub Username or Repository <span class="text-emerald-500 normal-case tracking-normal font-medium ml-0.5">*</span>
    </label>
    <input
      id="github-input"
      type="text"
      bind:value={username}
      placeholder="username or full profile/repo URL"
      class={I}
      spellcheck={false}
    />

<script lang="ts">
  ...
  const computed = $derived.by(() => {
    return buildSocialUrl(username, 'https://t.me/');
  });
...
<div class="space-y-5">
  <div class="space-y-1.5">
    <label for="telegram-input" class="block text-xs font-semibold uppercase tracking-wide text-muted-foreground">
      Telegram Username <span class="text-emerald-500 normal-case tracking-normal font-medium ml-0.5">*</span>
    </label>
    <input
      id="telegram-input"
      type="text"
      bind:value={username}
      placeholder="username or full t.me link"
      class={I}
      spellcheck={false}
    />

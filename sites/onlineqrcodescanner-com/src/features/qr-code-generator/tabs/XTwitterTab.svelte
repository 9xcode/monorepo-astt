<script lang="ts">
  ...
  const computed = $derived.by(() => {
    return buildSocialUrl(username, 'https://x.com/');
  });
...
<div class="space-y-5">
  <div class="space-y-1.5">
    <label for="x-twitter-input" class="block text-xs font-semibold uppercase tracking-wide text-muted-foreground">
      X (Twitter) Username <span class="text-emerald-500 normal-case tracking-normal font-medium ml-0.5">*</span>
    </label>
    <input
      id="x-twitter-input"
      type="text"

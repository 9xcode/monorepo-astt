<script lang="ts">
  ...
  const computed = $derived.by(() => {
    return buildSocialUrl(username, 'https://threads.net/@');
  });
...
<div class="space-y-5">
  <div class="space-y-1.5">
    <label for="threads-input" class="block text-xs font-semibold uppercase tracking-wide text-muted-foreground">
      Threads Username <span class="text-emerald-500 normal-case tracking-normal font-medium ml-0.5">*</span>
    </label>
    <input
      id="threads-input"
      type="text"

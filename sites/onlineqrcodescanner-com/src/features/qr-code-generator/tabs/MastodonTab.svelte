<script lang="ts">
  ...
  const computed = $derived.by(() => {
    return buildSocialUrl(username, 'https://mastodon.social/');
  });
...
<div class="space-y-5">
  <div class="space-y-1.5">
    <label for="mastodon-input" class="block text-xs font-semibold uppercase tracking-wide text-muted-foreground">
      Mastodon Username or URL <span class="text-emerald-500 normal-case tracking-normal font-medium ml-0.5">*</span>
    </label>
    <input
      id="mastodon-input"
      type="text"
      bind:value={username}
      placeholder="@username@instance.social or full profile URL"
      class={I}
      spellcheck={false}
    />

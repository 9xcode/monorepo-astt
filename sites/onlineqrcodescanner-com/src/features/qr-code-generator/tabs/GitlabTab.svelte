<script lang="ts">
  ...
  const computed = $derived.by(() => {
    return buildSocialUrl(username, 'https://gitlab.com/');
  });
...
<div class="space-y-5">
  <div class="space-y-1.5">
    <label for="gitlab-input" class="block text-xs font-semibold uppercase tracking-wide text-muted-foreground">
      GitLab Username or Project <span class="text-emerald-500 normal-case tracking-normal font-medium ml-0.5">*</span>
    </label>
    <input
      id="gitlab-input"
      type="text"
      bind:value={username}
      placeholder="username or full project URL"
      class={I}
      spellcheck={false}
    />
